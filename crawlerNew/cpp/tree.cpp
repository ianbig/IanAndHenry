...
std::string get_title(pt::ptree node) {
    pt::ptree nnode;
    pt::ptree nnnode;
    //find html
    for(auto it = node.begin(); it != node.end(); it++ ) {
        if(it->first == "html") {
            nnode = it->second; // find head
            for(auto iit = nnode.begin(); iit != nnode.end(); iit++ ) {
                if(iit->first == "head") {
                    nnnode = iit->second; // find title
                    for( auto iiit = nnnode.begin(); iiit != nnnode.end(); iiit++) {
                        if(iiit->first == "title") {
                            return iiit->second.data();
                        }
                    }
                }
            }
