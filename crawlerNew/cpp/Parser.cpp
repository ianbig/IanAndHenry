...
int Parser::fetch(const char *url, int news_type) {
    CURL *fetcher = NULL;
...
int Parser::mainTexExtraction(char *buffer, char *url) {
    ...
    // copy title
    tmp_chunk.title = new char[title.length() + 1];
    memmove(tmp_chunk.title, title.c_str(), title.length());
    tmp_chunk.title[title.length()] = 0;
...
void Parser::write_out_record(std::string filePath) {
    ...
    for(int i = 0; i < RecordQueue.size(); i++) {
        ...
        fout << "@\n" << "@REC:" << std::endl << "@url:" << chunk.url << std::endl << "@title:" << chunk.title << std::endl <<"@body:" << chunk.buffer << std::endl;
    }
    fout.close();
